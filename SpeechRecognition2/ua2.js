...
function ua_start(){
    ...
    config['displayName'] = getParameterByName("displayname");
    config['uri'] = getParameterByName("uri");
    config['authorizationUser'] = getParameterByName("authname");
    config['password'] = getParameterByName("password");
    config['wsServers'] = getParameterByName("wss");
    config['displayName'] = getParameterByName("invite");
...
//        elements.registerButton.innerHTML = 'Unregister';
        ...
        var sip_uri = getParameterByName("invite");
        console.log(sip_uri);
        if(callalready ==0){
            var session = ua.invite(sip_uri, {
                mediaConstraints: {
                    audio: true,
                    video: false
                }
            ...
            var ui = createNewSessionUI(sip_uri, session);
            callalready = 1;
        }
